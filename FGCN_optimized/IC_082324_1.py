...
formulation_file = "train_combdata_082324.csv"
...
#electrolyte_model = keras.Model(inputs=[node0,adj0,node1,adj1,node2,adj2,node3,adj3,node4, adj4, node5, adj5,wt], outputs=capacity)

electrolyte_model.summary()
# electrolyte_model =  tf.keras.models.load_model('ckpt/IC_060724_5-620-0.0146')

keras.utils.plot_model(electrolyte_model, modelname+"_struct.png", show_shapes=True)
...
# dataset = dataset.shuffle(int(len(formulation_indx)))
# val_data = dataset.take(int(len(formulation_indx)*0.1))
# train_data = dataset.skip(int(len(formulation_indx)*0.1)).take(int(len(formulation_indx)*0.9))


## Checkpoint 
checkpoint_filepath = 'ckpt/' + modelname + '-{epoch:02d}-{loss:.4f}'
...
result = electrolyte_model.fit(
    dataset.batch(1), epochs=epochs, shuffle=True, verbose=2,callbacks=[model_checkpoint_callback])
...
# checkpoint_filepath = 'ckpt/TL_model_IC-02-0.0208.model.keras'
